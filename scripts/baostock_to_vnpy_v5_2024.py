"""
baostock -> vnpy SQLite 2024年重试脚本
用途：重新导入2024年失败的2434只股票（含DB写入重试）
"""
...
MAX_RETRIES = 3
RETRY_DELAY = 2
DB_RETRY_MAX = 5         # DB写入最大重试
DB_RETRY_DELAY = 3       # DB写入重试间隔（秒）
CACHE_FILE = "/Users/zyb/go/src/github/vnpy/.cache/stocks_akshare.json"
PROGRESS_FILE = "/tmp/baostock_progress_2024_retry.json"
LOG_FILE = "/tmp/baostock_v5_2024_retry.log"
...
def main():
    print(f"=== baostock 2024年重试 -> vnpy SQLite ===")
    ...
    # 只处理未完成的：从 failed 中排除已 done 的 + 从未处理过的
    # failed_set 中可能有既在 done 又在 failed 的（之前失败后又成功了）
    retry_failed = failed_set - done_set
    unprocessed = set(all_stocks) - done_set - failed_set
    to_import = sorted(retry_failed | unprocessed)

    print(f"  全部股票: {len(all_stocks)}")
    print(f"  已完成: {len(done_set)} 只")
    print(f"  之前失败(需重试): {len(retry_failed)} 只")
    print(f"  未处理: {len(unprocessed)} 只")
    print(f"  本次待处理: {len(to_import)} 只")
    ...
    print(f"\n{'='*50}")
    print(f"  全部股票: {len(all_stocks)}")
    print(f"  本次导入: {total_bars} 条 ({len(new_done - done_set)} 只)")
