"""
援军战法 全市场回测 v0
基于 sector_backtest.py 修改，移除板块池限制。
C选股改为按回调深度排序（跌得越深越优先），取前50%。
"""
import os
import sqlite3
import sys
...
DATE_END = "2026-05-11"
DATA_LOAD_START = "2024-10-01"  # 缩短预加载范围，只需60天数据
INITIAL_CAPITAL = 1_000_000.0

# 策略参数（保持与 sector_backtest 一致）
COOLDOWN_DAYS = 0
RR_RATIO = 0.5
MAX_TRADES_PER_DAY = 20
DOWN_AMPLITUDE = 0.10        # 回调 ≥ 10%
STOP_LOSS_PCT = 0.10
MAX_HOLD_DAYS = 30
REL_STRENGTH_TOP_PCT = 0.5   # C: 超额收益前50%
ATR_TARGET_MULTIPLIER = 3.0
MAX_CONCURRENT_POSITIONS = 8
MAX_POSITIONS_PER_SECTOR = 8  # 全市场只有一个板块，设在和总上限一致
CIRCUIT_BREAKER_DRAWDOWN = 0.10
CIRCUIT_BREAKER_RESUME = 0.03
...
def load_all_from_db(
    ...
    query = f"""
        WITH daily AS (
            ...
            FROM dbbardata
            WHERE interval = '5m'
                AND datetime >= '{DATA_LOAD_START} 09:00:00'
    ...
    # 按股票拆分，过滤不足60天（年线已注释，60天足够满足其他条件）
...
class MarketBacktest:
    ...
    def __init__(
        ...
        self.config = StrategyConfig(
            ...
            risk_config=RiskConfig(
                ...
                atr_multiplier=2.0,
                max_consecutive_losses=999,  # 去掉连亏暂停
            ),
        ...
        # 缓存：每日全市场5日平均涨幅
        self._market_avg_5d_cache: Dict[datetime, float] = {}
    ...
    def run(self, date_start: str, date_end: str) -> pd.DataFrame:
        ...
        print(f"  形态识别: 回调≥{DOWN_AMPLITUDE*100:.0f}%, |日涨跌|≤6%, 距撤军线≤2%, 年线已注释, 冷却{COOLDOWN_DAYS}天")
        print(f"  撤军线=10日最低价 | C选股: 超额收益前{REL_STRENGTH_TOP_PCT*100:.0f}%")
        print(f"  止损线=撤军线 | 持仓≤{MAX_HOLD_DAYS}天 | 每日上限{MAX_TRADES_PER_DAY}笔")
        print(f"  敞口上限: 总{MAX_CONCURRENT_POSITIONS}笔")
        print(f"  熔断: 已注释")
        print("-" * 60)
        ...
        for t_idx, date in enumerate(test_dates):
            ...
            # 预计算当日全市场5日平均涨幅
            self._market_avg_5d_cache[date] = self._calc_market_avg_5d(date)
        ...
        actual_enter = (self._stat_candidates - self._stat_no_data
                        - self._stat_bottom_fail - self._stat_bottom_cooldown
                        - self._stat_entry_fail - self._stat_pos_zero)
        print(f"  实际入场:       {actual_enter:>6,}")

        return pd.DataFrame(self.equity_curve)

    def _check_exits(self, date) -> None:
        """检查持仓退出（撤军线固定=入场时10日最低价）"""
    ...
    def _calc_market_avg_5d(self, date: datetime) -> float:
        """预计算当日全市场平均5日涨幅（每日调用一次）"""
        all_rets = []
        for sym, df in self.stock_data.items():
            ...
            if len(sub) < 6:
                continue
            pc = float(sub["close"].iloc[-6])
            if pc <= 0:
                continue
            ret = (float(sub["close"].iloc[-1]) - pc) / pc
            all_rets.append(ret)
        return mean(all_rets) if all_rets else 0.0

    def _calc_relative_strength(self, hist: pd.DataFrame, date: datetime) -> float:
        """计算个股在全市场的相对强度（5日超额收益，使用缓存）"""
        if len(hist) < 6:
            return 0.0
        ...
        market_avg = self._market_avg_5d_cache.get(date, 0.0)
        return stock_5d_ret - market_avg
    ...
    def _check_entries(self, date) -> None:
        ...
        for code in self.stock_data:
            ...
            # C: 相对强度（5日超额收益）
            rel_strength = self._calc_relative_strength(hist, date)
        ...
        # C: 按回调深度排序取前50%（跌得最深的优先入场）
        candidates.sort(key=lambda x: x["rel_strength"], reverse=True)
        top_n = max(1, int(len(candidates) * REL_STRENGTH_TOP_PCT))
        selected = candidates[:top_n]
        filtered_out = len(candidates) - top_n
        ...
        for c in selected:
            ...
            self.positions[c["code"]] = {
                ...
                "cooldown_interrupted": c["bottom_info"].get("cooldown_interrupted", False),
            }
...
def main():
    ...
    print("  参数: 冷却0天 | 撤军线=10日低点 | ATR*3目标 | 持仓30天 | 全局C选股")
    print("=" * 60)

    # 加载所有股票
    stock_data, _ = load_all_from_db(DATE_START, DATE_END)
    ...
    if bt.trade_records:
        ...
        for r in bt.trade_records:
            print(f"  {r.entry_date} → {r.exit_date} | {r.vt_symbol} | "
                  f"{'赢' if r.is_win else '亏'} | {r.exit_reason} | "
                  f"{r.pnl_pct*100:.1f}% | {r.hold_days}天")

    # 保存
    out_path = f"{PROJECT_ROOT}/.cache/market_backtest_2024_2026.pkl"
