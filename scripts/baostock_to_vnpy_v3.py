"""
baostock -> vnpy SQLite 全量A股数据导入
优化依据：
  - 官方文档：分钟级数据 QPS ≈ 20，建议 time.sleep(0.5)
  - 实测发现：连续 ~50 只后会话级断流，需定期重登录清理状态
"""
...
START_DATE = "2026-02-10"    # 近3个月（baostock分钟K线上限）
END_DATE = datetime.today().strftime("%Y-%m-%d")
BS_FREQ = "5"
VT_INTERVAL = Interval.MINUTE5    # ← 修复：5分钟K线用 MINUTE5，存为 "5m"
QUERY_DELAY = 0.5             # 官方推荐：≥ 500ms
BATCH_SIZE = 50              # 每批提交数
RELOGIN_EVERY = 50           # 每 N 只重登录一次，防止会话级断流
MAX_RETRIES = 3               # 单只股票最大重试（重登录后）
...
def main():
    print(f"=== baostock 全量A股 -> vnpy SQLite ===")
    ...
    try:
        ...
        for i, bs_sym in enumerate(to_import):
            ...
            # 每 RELOGIN_EVERY 只重登录一次（清理会话状态，防止断流）
